...
//Check if the player has the X_X aspect
bool CheckPlayerAspect()
{
     if (player == "(X_X)") {
        return true;
    } else {
...
//Check if the player has the  (^-^) aspect
bool CheckPlayerAspect2()
{
     if (player == "(^-^)") {
        return true;
    } else {
...
void Move(bool optional = false, bool moreSpeed = false) 
{
    int lastX = playerX;
    int lastY = playerY;
    var keyPulsed = Console.ReadKey(true).Key;

    if(optional) {
        if(keyPulsed != ConsoleKey.UpArrow && keyPulsed != ConsoleKey.DownArrow && keyPulsed != ConsoleKey.RightArrow && keyPulsed != ConsoleKey.LeftArrow) {
            shouldExit = true;
        }
    }

    if (CheckPlayerAspect()) {
        FreezePlayer();
    }

    if (CheckPlayerAspect2()) {
        moreSpeed = true;
    }
